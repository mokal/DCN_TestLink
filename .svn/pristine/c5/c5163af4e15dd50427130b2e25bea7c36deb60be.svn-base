...
class testcase extends tlObjectWithAttachments
{
  ...
  */
  function get_by_name($name, $tsuite_name = '', $tproject_name = ''){
   ...
   function modifyTplanDeviceCases($tplanid, $deviceid, $suite, $casename, $operate,$user_name){
   if($suite == 'testlink'){ //$casename is tcversion script
    	$sql =  " SELECT id FROM users WHERE login = '{$user_name}' " ;
    	$userid = $this->db->fetchFirstRow($sql);
		$userid = $userid['id'];
   	
   		$sql =  " SELECT id FROM tcversions WHERE preconditions = '{$casename}' " ;
   		$tcversionid = $this->db->fetchFirstRow($sql);
   		$tcversionid = $tcversionid['id'];
   		$returnrs = 'Success' ;
   		if($tcversionid != null){
   			switch($operate){
   				case "add":
   					$sql =  " INSERT INTO testplan_tcversions (`testplan_id`,`tcversion_id`,`platform_id`,`author_id`) VALUES ({$tplanid},{$tcversionid},{$deviceid},{$userid}) " ;
   					$this->db->exec_query($sql);
   					break;
   				case "delete":
   					$sql =  " DELETE FROM testplan_tcversions WHERE testplan_id = {$tplanid} AND platform_id = {$deviceid} AND tcversion_id = {$tcversionid} " ;
   					$this->db->exec_query($sql);
   					break;
   				case "deactive":
   					$sql =  " UPDATE testplan_tcversions SET active=0 WHERE testplan_id = {$tplanid} AND platform_id = {$deviceid} AND tcversion_id = {$tcversionid} " ;
   					$this->db->exec_query($sql);
   					break;
   				default:
   					$returnrs = 'Fail:operate is not add or delete!' ;
   					break;
   			}
   		}else{
   			$returnrs = 'Fail!' ;
    ...
    $sql =  " SELECT testproject_id FROM testplans WHERE id = {$tplanid} " ;
    $rs = $this->db->fetchFirstRow($sql);
    $rs = $rs['testproject_id']; 
    $sql =  " SELECT name FROM nodes_hierarchy WHERE id = {$rs} " ;
    $rs = $this->db->fetchFirstRow($sql);
    $productline_name = $rs['name'];

    $sql =  " SELECT id FROM users WHERE login = '{$user_name}' " ;
    $userid = $this->db->fetchFirstRow($sql);
	$userid = $userid['id'];

     $testcase =  $this->get_by_name($casename, $suite , $productline_name);
     if( is_null($testcase) ){
         return 'Fail:please ADD test case first!'; 
     }else{
        $sql =  " SELECT id FROM nodes_hierarchy WHERE parent_id = {$testcase[0]['id']} " ;
        $tcv_id = $this->db->fetchFirstRow($sql);
        $tcv_id = $tcv_id['id'];
     }
     $returnrs = 'Success' ;
     switch($operate){
         case "add":
              $sql =  " INSERT INTO testplan_tcversions (`testplan_id`,`tcversion_id`,`platform_id`,`author_id`) VALUES ({$tplanid},{$tcv_id},{$deviceid},{$userid}) " ;
              $this->db->exec_query($sql);
              break;
         case "delete":
              $sql =  " DELETE FROM testplan_tcversions WHERE testplan_id = {$tplanid} AND platform_id = {$deviceid} AND tcversion_id = {$tcv_id} " ;
              $this->db->exec_query($sql);
              break;
         case "deactive":
            	$sql =  " UPDATE testplan_tcversions SET active=0 WHERE testplan_id = {$tplanid} AND platform_id = {$deviceid} AND tcversion_id = {$tcv_id} " ;
              	$this->db->exec_query($sql);
              	break;
         default:
              $returnrs = 'Fail:operate is not add or delete!' ;
  ...
  */
  function get_linked_versions($id,$filters=null,$options=null)
  {
    ...
    $fields2get = 'tc_external_id,version,status,importance,tcversions.active, is_open,execution_type,';
...
//
 function getExecutionSetBuild($tplan_id,$build_id,$id)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $my['filters'] = array('tcversion_id' => null,'testplan_id' => $tplan_id,
                            'platform_id' => null, 'build_id' => $build_id);  
...
//
 function getExecutionSetBuildPlatform($tplan_id,$build_id,$device_id,$id)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $my['filters'] = array('tcversion_id' => null,'testplan_id' => $tplan_id,
                            'platform_id' => $device_id, 'build_id' => $build_id);  
 ...
 function getExecutionSetTplanPlatform($tplan_id,$device_id,$id)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $my['filters'] = array('tcversion_id' => null,'testplan_id' => $tplan_id,
                            'platform_id' => $device_id, 'build_id' => null);  
  ...
  function getLatestExecSingleContext($identity,$execContext,$options=null)
  {
    ...
    {
      case 'exec_id':
          $mysql = ' SELECT id FROM executions ' . 
              ' WHERE testplan_id = ' . $safeContext['tplan_id'] . 
              ' AND platform_id = ' . $safeContext['platform_id'] . 
              ' AND build_id = ' . $safeContext['build_id'] ;
           if( $safeContext['stack'] != 999 ) { $mysql = $mysql .  ' AND stack = ' . $safeContext['stack'] ;}
           if( $safeContext['job_id']!= '999' ) { $mysql = $mysql .  " AND job_id = '{$safeContext['job_id']}'" ;}
           $mysql = $mysql . ' ORDER BY id DESC ';

        $dummy = $this->db->get_recordset($mysql);
