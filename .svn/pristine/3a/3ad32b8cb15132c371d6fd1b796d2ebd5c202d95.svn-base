...
{foreach item=buildname from=$gui->build_name key=index}
版本{$index}:&nbsp&nbsp<span style="color:red">{$buildname}</span>&nbsp&nbsp&nbsp
{/foreach}
<strong></div>
<br>
...
<table class="simple_tableruler" style="text-align:left; margin-left: 2px;">
    <tHead>
    {if $gui->build_total ==2 }
    <tr><th rowspan='2' align="center" valign="middle" width=20>TestCase</th>
        <th rowspan='2' align="center" valign="middle" width=40>Summary</th>
        <th colspan="3" style="text-align:center;">版本一</th>
        <th colspan="3" style="text-align:center;">版本二</th>
    ...
    {if $gui->build_total !=2 }
        <tr>
        <th align="center" valign="middle" width=20>TestCase</th>
		<th align="center" valign="middle" width=40>Summary</th>
        {foreach item=buildid from=$gui->build key=index}		
            <th style="text-align:center;" width=10>版本{math equation="x + y" x=1 y=$index}</th>
	    {/foreach}
	    </tr>
    {/if}
    </tHead>
