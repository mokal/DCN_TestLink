...
class tlTestPlanMetrics extends testplan
{
   ...
   * @since 1.9.4
   **/
  function getOverallBuildStatusForRender_mokal($id){
  	$renderObj = null;
  	$code_verbose = $this->getStatusForReports();
  	$labels = $this->resultsCfg['status_label'];
  	
  	
  	$sql = " SELECT id as build_id,name FROM builds WHERE testplan_id={$id} AND name != 'Dynamic Create' " ;
  	$buildset = $this->db->fetchRowsIntoMap($sql,'build_id');
  	
  	$sql = " SELECT platform_id FROM testplan_platforms WHERE testplan_id={$id} " ;
  	$deviceset = $this->db->fetchRowsIntoMap($sql,'platform_id');
  	$devices = implode(",",array_keys($deviceset)); 

  	if( !is_null($buildset) && !is_null($deviceset) ){
  		$renderObj = new stdClass();
  		$renderObj->info = array();
  		foreach($buildset as $buildID=>$build)
  		{
  			$totalRun = 0;
  			$sql = " SELECT COUNT(id) as total_tptcv FROM testplan_tcversions " .
  				   " WHERE testplan_id={$id} AND platform_id IN ( {$devices} ) ";
  			$rs = $this->db->fetchFirstRow($sql);
  			
  			$renderObj->info[$buildID]['build_name'] = $build['name'];
  			$renderObj->info[$buildID]['total_assigned'] = $rs['total_tptcv'];
  			...
  			foreach($code_verbose as $statusCode => $statusVerbose)
  			{
  				...
  				foreach($deviceset as $deviceid=>$device){
  					$sql = " select tcversion_id,status from executions a " .
  						   " where execution_ts in (select max(execution_ts) from executions b where testplan_id={$id} and build_id={$buildID} and platform_id={$deviceid} and b.tcversion_id=a.tcversion_id) " .
  						   " and status='{$statusCode}' group by tcversion_id " ;
  					$rs = $this->db->get_recordset($sql);
  					if(!is_null($rs)){
  						$rf[$statusVerbose]['qty'] +=  count($rs);
  					}
  				}
  ...
  function getOverallBuildStatusForRender($id, $totalKey='total_assigned')
  {
  	...
  	$sql = " DELETE FROM user_assignments WHERE type=1 " ;
  	$this->db->exec_query($sql);

  	foreach( $buildset as $buildid=>$build ){
  		$sql = " SELECT id FROM testplan_tcversions WHERE testplan_id={$id} AND platform_id IN ( {$devices} ) " ;
  		$rs = $this->db->fetchRowsIntoMap($sql,'id');
  		$as = '';
  		$total = count($rs);
  		$index = 1;
  		foreach( $rs as $eid=>$temp){
  			if($index ==1){
  				$as  .= " ({$eid},{$buildid},1,1,1) " ;
  			}else{
  				$as .= " ,({$eid},{$buildid},1,1,1) ";
  			}
  			++$index;
  		...
  		$sql = " INSERT INTO user_assignments (feature_id,build_id,type,assigner_id,user_id) VALUES " . $as ;
  		$this->db->exec_query($sql);
  	}
