...
class build_mgr extends tlObject
{
  ...
  /*
    ...
    rev :
  */
  function create($tplan_id,$name,$result,$result_summary,$reviewer,$review_summary,$notes = '',$active=1,$open=1,$release_date='')
  {
     if($release_date == ''){
         $release_date = date("Y-m-d") ;
     }
    $targetDate=trim($release_date);
    $sql = " INSERT INTO {$this->tables['builds']} " .
           " (testplan_id,name,result,result_summary,reviewer,review_summary,notes,release_date,active,is_open,creation_ts) " .
           " VALUES ('". $tplan_id . "','" . $this->db->prepare_string($name) . "','" .$this->db->prepare_string($result) . "','" .$this->db->prepare_string($result_summary) . "','" .
              $this->db->prepare_string($reviewer) . "','" .$this->db->prepare_string($review_summary) . "','" .$this->db->prepare_string($notes) . "',";
  ...
  /*
    ...
    rev :
  */
  function update($id,$name,$notes,$result,$result_summary,$reviewer,$review_summary,$deviceid,$device_result,$device_result_summary,$device_reviewer,$device_review_summary,$topoid,$device_topo_result,$device_topo_result_summary,$device_topo_reviewer,$device_topo_review_summary,$active=null,$open=null,$release_date='',$closed_on_date='')
  {
    $closure_date = '';
    $targetDate=trim($release_date);
    $sql = " UPDATE {$this->tables['builds']} " .
           " SET name='" . $this->db->prepare_string($name) . "'," .
           "     result='" . $this->db->prepare_string($result) . "'," .
      ...
      if( $deviceid != 0 ){
            $temp =   " UPDATE build_device_result " .
                           " SET result='" . $this->db->prepare_string($device_result) . "'," .
                           "  result_summary='" . $this->db->prepare_string($device_result_summary) . "'," .
                           "  reviewer='" . $this->db->prepare_string($device_reviewer) . "'," .
                           "  review_summary='" . $this->db->prepare_string($device_review_summary) . " ' " .
                           "  WHERE device_id={$deviceid} AND build_id = {$id} ";
            $this->db->exec_query($temp);
       }

      if( $topoid != 999 ){
            $temp =   " UPDATE build_device_topo_result " .
                           " SET result='" . $this->db->prepare_string($device_topo_result) . "'," .
                           "  result_summary='" . $this->db->prepare_string($device_topo_result_summary) . "'," .
                           "  reviewer='" . $this->db->prepare_string($device_topo_reviewer) . "'," .
                           "  review_summary='" . $this->db->prepare_string($device_topo_review_summary) . " ' " .
                           "  WHERE device_id={$deviceid} AND build_id = {$id} AND topo_type={$topoid}";
            $this->db->exec_query($temp);
       }
  ...
  if( $role != 8 ){
    ...
    $sql = " DELETE FROM build_device_result {$where} ";
    $this->db->exec_query($sql);

    $sql = " DELETE FROM build_device_topo_result {$where} ";
    $this->db->exec_query($sql);
