...
class TestlinkXMLRPCServer extends IXR_Server
{
  ...
  protected function _insertResultToDB($user_id=null)
  {
    ...
    $stack = isset($this->args['stack'])? $this->args['stack'] : 0 ;
    $jobid = isset($this->args['job_id'])? $this->args['job_id'] : '';
    ...
    $query = "INSERT INTO {$this->tables['executions']} " .
             ...
             " platform_id, tcversion_number,stack, job_id," .
             " execution_type {$notes_field} ) " .
             " VALUES({$build_id},{$tester_id},{$db_now},'{$status}',{$testplan_id}," .
             " {$tcversion_id},{$platform_id}, {$version_number},{$stack},'{$jobid}',{$execution_type} {$notes_value})";

    $this->dbObj->exec_query($query);
    $exec_id = $this->dbObj->insert_id($this->tables['executions']);

    $sql = " UPDATE running_jobs SET topo_type={$stack} WHERE job_id = '{$jobid}' ";
    $this->dbObj->exec_query($sql);
     ...
     if( isset($this->args['job_id']) ){
        $this->platformMgr->reportJobResult($this->args['job_id'], $exec_id);
     }
     return $exec_id ;
  }
  ...
  public function createJob($args)
  {
    ...
    $sql = "SELECT id FROM {$this->tables['nodes_hierarchy']} WHERE name='{$productLine}' AND node_type_id=1 " ;
    $productline_id = $this->dbObj->fetchFirstRow($sql);
     if( !$productline_id ){
         $resultInfo[0]["status"] = 'false';
         $resultInfo[0]["message"] = 'productline not exist!';
         return $resultInfo;
     }

    $sql = "SELECT id FROM {$this->tables['nodes_hierarchy']} WHERE name='{$testPlan}' AND node_type_id=5 " ;
    $tplan_id = $this->dbObj->fetchFirstRow($sql);
    if( !$tplan_id ){
        $resultInfo[0]["status"] = false;
        $resultInfo[0]["message"] = 'test plan not exist!';
        return $resultInfo;
    }

    $sql = "SELECT id FROM {$this->tables['platforms']} WHERE name='{$testDevice}' " ;
    $device_id = $this->dbObj->fetchFirstRow($sql);
    if( !$device_id ){
        $resultInfo[0]["status"] = false;
        $resultInfo[0]["message"] = 'test device not exist!';
        return $resultInfo;
    }
    ...
    if( !$user_id ){
        $resultInfo[0]["status"] = false;
        $resultInfo[0]["message"] = 'user not exist!';
        return $resultInfo;
    }

    $jobid = "Daily-" . (string)$args['user'] . (string)time();
...
//add by guomf  
  public function getTestCaseIDByNameLike($args)
  {
    ...
    {      
      ...
      {
        $result = $testCaseMgr->get_by_namelike($testCaseName,$optional[self::$testSuiteNameParamName],
                                            $optional[self::$testProjectNameParamName]);
      }
  ...
  {    
    ...
    { 
      if( $this->_isParamPresent(self::$userParamName) )
      {
        ...
        if( !($status_ok = !is_null($tester_id)) )
        {
          ...
          $this->errors[0] = new IXR_Error(NO_USER_BY_THIS_LOGIN, $msg);  
        }
      }
    }
    ...
    { 
      ...
      if($this->_isParamPresent(self::$overwriteParamName) && $this->args[self::$overwriteParamName])
      {
      	...
      	$sql = " SELECT id FROM {$this->tables['executions']} WHERE " .
      		   " job_id='{$jobid}' AND tcversion_id={$tcversion_id} AND testplan_id={$testplan_id} AND platform_id={$platform_id} AND build_id={$build_id} AND stack={$stack} ";
      	$rs=$this->dbObj->get_recordset($sql);
      	if( is_null($rs) ){
      		$executionID = 0 ;
      	}else{
      		$query = " UPDATE {$this->tables['executions']} SET status='{$status}' WHERE id={$rs[0]['id']} ";
      		$this->dbObj->exec_query($query);
      		...
      		if( isset($this->args['job_id']) ){
      			$this->platformMgr->reportJobResult($this->args['job_id'], $executionID);
      		}      		
      	}
      }
      if($executionID == 0)
  ...
  public function getIssueInfo($args)
  {
    if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    }
    return $this->platformMgr->matchIssueByID($args['testProjectID'], $args['product'], $args['script'], $args['testcase'], $args['step']);
	}
	
   public function getOldDiff($args)
	{
		if( is_null($this->platformMgr) )
		{
			$this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
		}
		return $this->platformMgr->getOldDiff($args['jobid'],$args['tcid']);
	}
 
   public function getJobInfo($args)
  {
    if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    ...
    $rs =  $this->platformMgr->getJobInfo($args['jobid']);
    return $rs; 
   }

  public function getJobEnv($args)
  {
    if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    }
    return $this->platformMgr->getJobEnv($args['jobid']);
	}

  public function getJobCases($args)
  {
    if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    }
    return $this->platformMgr->getJobCases($args['jobid']);
	}

  public function getTplanTestCases($args)
  {
    if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    }
    return $this->platformMgr->getTplanTestCases($args['testplanid']);
	}

  public function getTplanDevices($args)
  {
    if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    }
    return $this->platformMgr->getTplanDevices($args['testplanid']);
	}

  public function updateJobInfo($args)
  {
    if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    }
    return $this->platformMgr->updateJobInfo($args['jobid'], $args['status'], $args['case']);
	}

  public function returnCreatedEnvDetails($args)
	{
		if( is_null($this->platformMgr) )
		{
			$this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
		}
		return $this->platformMgr->returnCreatedEnvDetails($args['jobid'], $args['details']);
	}
	
  public function updateJobBuild($args)
  {
    if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    }
    return $this->platformMgr->updateJobBuild($args['jobid'], $args['id']);
	}

  public function modifyBuildName($args)
  {
    if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    }
    return $this->platformMgr->modifyBuildName($args['jobid'], $args['version']);
	}

  public function reportJobResult($args)
  {  
   return 1;
	}

  public function reportJobResult_new($args)
  {  
   if( is_null($this->platformMgr) )
    {
      $this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
    ...
    return $this->platformMgr->reportJobResult($args['jobid'], $args['exeid']);
	}

	public function reportJobWorker($args)
	{
		if( is_null($this->platformMgr) )
		{
			$this->platformMgr = new tlPlatform($this->dbObj,$testProjectID);
		}
		return $this->platformMgr->reportJobWorker($args['jobid'], $args['ip']);
	}
  /**
...
public function getTestCase($args)
{
  ...
  protected function _updateResult($user_id=null)
  {
    $exec_id = 0;
    $tester_id =  is_null($user_id) ? $this->userID : $user_id;
    $status = $this->args[self::$statusParamName];
    ...
    $sql = " SELECT MAX(id) as id FROM executions WHERE testplan_id = {$this->args[self::$testPlanIDParamName]} " .
           " AND build_id = {$this->args[self::$buildIDParamName]} " .
           " AND platform_id = {$this->args[self::$platformIDParamName]} " ;

    $stack_update = '' ;
    $job_id_update = '' ;

    if(isset($this->args['stack'])){
    	$stack_update = " ,stack='{$this->args['stack']}' ";
    	$sql .= " AND stack='{$this->args['stack']}' ";
    }
    if(isset($this->args['job_id'])){
    	$job_id_update = " ,job_id='{$this->args['job_id']}' ";
    	$sql .= " AND job_id='{$this->args['job_id']}' ";
    }
   ...
   //                                                       $execContext, $opt);
    if( $exec_id != 0 )
    {
      ...
      $sql = " UPDATE {$this->tables['executions']} SET tester_id={$tester_id}, execution_ts={$db_now}, " . 
             " status='{$status}', execution_type= {$execution_type} " . 
             " {$notes_update} {$stack_update} {$job_id_update} WHERE id = {$exec_id}";

      $this->dbObj->exec_query($sql);

      if( isset($this->args['stack']) && isset($this->args['job_id']) ){
          $sql = " UPDATE running_jobs SET topo_type='{$this->args['stack']}' WHERE job_id = '{$this->args['job_id']}' ";
          $this->dbObj->exec_query($sql);
      }
      if( is_null($this->platformMgr) )
      {
  ...
  function initMethodYellowPages()
  {
    $this->methods = array( 'tl.reportTCResult' => 'this:reportTCResult',
                            ...
                            'tl.getTestCaseIDByName' => 'this:getTestCaseIDByName',
		              		'tl.getTestCaseIDByNameLike' => 'this:getTestCaseIDByNameLike',
                            ...
                            'tl.getIssueInfo' => 'this:getIssueInfo',
                    		'tl.getOldDiff' => 'this:getOldDiff',
                            'tl.getJobInfo' => 'this:getJobInfo',
                            'tl.createJob' => 'this:createJob',
                            'tl.getJobEnv' => 'this:getJobEnv',
                            'tl.getJobCases' => 'this:getJobCases',
                            'tl.updateJobInfo' => 'this:updateJobInfo',
    						'tl.returnCreatedEnvDetails' => 'this:returnCreatedEnvDetails',
                            'tl.updateJobBuild' => 'this:updateJobBuild',
			    			'tl.modifyBuildName' => 'this:modifyBuildName',
			    			'tl.getTplanTestCases' => 'this:getTplanTestCases',
			    			'tl.getTplanDevices' => 'this:getTplanDevices',
			    			'tl.getBranchCases' => 'this:getBranchCases',
                            'tl.reportJobResult' => 'this:reportJobResult',
    						'tl.reportJobWorker' => 'this:reportJobWorker',
                            'tl.reportJobResult_new' => 'this:reportJobResult_new',
