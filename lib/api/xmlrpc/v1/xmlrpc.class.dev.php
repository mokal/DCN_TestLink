...
return '1123';
    ...
    $query = "INSERT INTO {$this->tables['executions']} " .
             ...
             " {$tcversion_id},{$platform_id}, {$version_number},{$stack},'{$job_id}',{$execution_type} {$notes_value})";
  ...
  public function createJob($args)
  {
    ...
    $jobid = (string)$args['user'] . (string)time();
    ...
    $this->platformMgr->addJob($jobid, 'affirm2', $total_case, $productline_id['id'], $tplan_id['id'], $device_id['id'], $build_id['id'], $args['user'], $args['vdi_ip']);
...
//add by guomf  
  ...
  {    
    ...
    { 
      ...
      if($executionID == 0)
      { return $this->_insertResultToDB($tester_id); 
  ...
  protected function _updateResult()
  {
    ...
    if( !is_null($exec_id) )
    {
      ...
      $sql = " UPDATE {$this->tables['executions']} " .
             " SET tester_id={$tester_id}, execution_ts={$db_now},stack={$stack},job_id='{$jobid}' ," . 
             " status='{$status}', execution_type= {$execution_type} " . 
             " {$notes_update}  WHERE id = {$exec_id}";
      
      $this->dbObj->exec_query($sql);

      $sql = " UPDATE {$this->tables['running_jobs']} " .
              " SET topo_type={$stack} WHERE job_id = '{$jobid}' ";
