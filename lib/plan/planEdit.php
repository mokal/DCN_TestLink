...
if($args->do_action == "do_create" || $args->do_action == "do_update")
{
  ...
  $gui->testplan_script_tag = $args->testplan_script_tag;
...
switch($args->do_action)
{
  ...
  case 'do_update':
    ...
    {
      if(!$tplan_mgr->update($args->tplan_id,$args->testplan_name,$args->testplan_script_tag,$args->notes,
  ...
  case 'do_create':
    ...
    if(!$name_exists)
    {
      $new_tplan_id = $tplan_mgr->create($args->testplan_name,$args->testplan_script_tag,$args->notes,
...
function init_args($request_hash)
{
  ...
  $nullable_keys = array('testplan_name','testplan_script_tag','notes','rights','active','do_action');
...
function getItemData(&$itemMgr,&$guiObj,&$ofObj,$itemID,$updateAttachments=false)
{
  ...
  if (sizeof($dummy))
  {
    ...
    $guiObj->testplan_script_tag = $dummy['script_tag'];
