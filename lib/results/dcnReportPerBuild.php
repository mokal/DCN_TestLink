...
if( !is_null($gui->platformSet) )
{
    ...
    foreach($gui->platformSet_flip as $deviceid){
       $results_temp = $tplan_mgr->get_exec_device($args->tplan_id,$build_id,$deviceid);
       $gui->result_total[$deviceid] = count($results_temp);
       foreach($results_temp as $index){
                 switch($index['status']){
                     case "Pass" :
       ...
       $gui->testresults= array_merge($gui->testresults, $results_temp);
      }
}
