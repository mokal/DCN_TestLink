...
switch ($doc_info->type)
{
  ...
  case DOC_TEST_PLAN_EXECUTION_ON_BUILD:
    ...
    if($args->build_id > 0)
    {
      //add by guomf for report custome
      $tempa = $tplan_mgr->get_build_testresult($args->build_id);
      $tempb = $tplan_mgr->get_build_resultsummary($args->build_id);
      $doc_info->testresult = htmlspecialchars($tempa['value']);
      $doc_info->resultsummary = htmlspecialchars_decode($tempb['value']); //htmlspecialchars($tempb['value']);
    ...
    //modify by guomf for print result on special build
    $platforms = $tplan_mgr->getPlatforms($args->tplan_id,$getOpt);   
    //$platforms = $tplan_mgr->getPlatformsHasExecuted($args->tplan_id,$args->build_id,$getOpt);   
